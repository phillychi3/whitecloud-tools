<script lang="ts">
  ...
  let result = $state<number[][]>([[0, 0, 0], [0, 0, 0], [0, 0, 0]]);
  ...
  const templateModules = import.meta.glob('$lib/assets/ff14_mini-cactpot/*.png', { eager: true, import: 'default' });
...
;
  ...
  function preprocessForOCR(canvas: HTMLCanvasElement, row: number, col: number): HTMLCanvasElement {
    const ctx = canvas.getContext('2d')!;
    const imageData = ctx.getImageData(0, 0, canvas.width, canvas.height);
    const data = imageData.data;


    const targetR = 100, targetG = 36, targetB = 36;
    const tolerance = 70;
  ...
  function findConnectedRegions(binary: Uint8Array, width: number, height: number): Region[] {
    ...
    function floodFill(startIdx: number): Region {
      ...
      while (stack.length > 0) {
        ...
        [idx - width - 1, idx - width, idx - width + 1, idx - 1, idx + 1,
         idx + width - 1, idx + width, idx + width + 1].forEach(n => {
  ...
  function recognizeCircle(ctx: CanvasRenderingContext2D, circle: Circle, row: number, col: number): number {
    ...
    cropCtx.drawImage(
      ctx.canvas,
      circle.x - size/2, circle.y - size/2, size, size,
      0, 0, size, size
    );
    cropCtx.restore();
    ...
    const processedImageData = processedCanvas.getContext('2d')!.getImageData(0, 0, processedCanvas.width, processedCanvas.height);
    ...
    <div class="bg-white rounded-lg shadow-md p-6">
      ...
      <div class="grid grid-cols-5 gap-1 w-fit">
        {#each [1] as _}
          ...
          <div class="w-16 h-16 flex flex-col items-center justify-center text-xs {isDiag1Max ? 'bg-yellow-100 border-2 border-yellow-500 font-bold' : 'bg-gray-50'} border border-gray-300 rounded">
            <Icon icon="mdi:arrow-bottom-right" class="w-4 h-4 text-gray-600" />
            {#if diag1Score > 0}
              <div class="{isDiag1Max ? 'text-yellow-700 font-bold' : 'text-blue-600'} text-[11px]">{diag1Score}</div>
            {:else if diag1Expected > 0}
              <div class="{isDiag1Max ? 'text-yellow-600' : 'text-gray-500'} text-[10px]">{diag1Expected}</div>
            {:else}
        ...
        {#each [0, 1, 2] as col}
          ...
          <div class="w-16 h-16 flex flex-col items-center justify-center text-xs {isMax ? 'bg-yellow-100 border-2 border-yellow-500 font-bold' : 'bg-gray-50'} border border-gray-300 rounded">
            <Icon icon="mdi:arrow-down" class="w-4 h-4 text-gray-600" />
            {#if colScore > 0}
              <div class="{isMax ? 'text-yellow-700 font-bold' : 'text-blue-600'} text-[11px]">{colScore}</div>
            {:else if colExpected > 0}
              <div class="{isMax ? 'text-yellow-600' : 'text-gray-500'} text-[10px]">{colExpected}</div>
            {:else}
              <div class="text-gray-400 text-xs">-</div>
            {/if}
          </div>
        ...
        {#each [1] as _}
          ...
          <div class="w-16 h-16 flex flex-col items-center justify-center text-xs {isDiag2Max ? 'bg-yellow-100 border-2 border-yellow-500 font-bold' : 'bg-gray-50'} border border-gray-300 rounded">
            <Icon icon="mdi:arrow-bottom-left" class="w-4 h-4 text-gray-600" />
            {#if diag2Score > 0}
              <div class="{isDiag2Max ? 'text-yellow-700 font-bold' : 'text-blue-600'} text-[11px]">{diag2Score}</div>
            {:else if diag2Expected > 0}
              <div class="{isDiag2Max ? 'text-yellow-600' : 'text-gray-500'} text-[10px]">{diag2Expected}</div>
            {:else}
              <div class="text-gray-400 text-xs">-</div>
            {/if}
          </div>
        ...
        {#each [0, 1, 2] as rowIndex}
          ...
          <div class="w-16 h-16 flex flex-col items-center justify-center text-xs {isMax ? 'bg-yellow-100 border-2 border-yellow-500 font-bold' : 'bg-gray-50'} border border-gray-300 rounded">
            ...
            {#if rowScore > 0}
              <div class="{isMax ? 'text-yellow-700 font-bold' : 'text-blue-600'} text-[11px]">{rowScore}</div>
            {:else if rowExpected > 0}
              <div class="{isMax ? 'text-yellow-600' : 'text-gray-500'} text-[10px]">{rowExpected}</div>
            {:else}
              <div class="text-gray-400 text-xs">-</div>
            {/if}
          </div>
          {#each result[rowIndex] as cell, colIndex}
            <input
              ...
              class="w-16 h-16 border-2 border-gray-400 text-center text-2xl font-bold {cell === 0 ? 'bg-gray-100 text-gray-400' : 'bg-white text-gray-800'} focus:outline-none focus:ring-2 focus:ring-blue-500 focus:bg-blue-50 rounded"
              placeholder="-"
              onkeypress={(e) => {
                if (!/[0-9]/.test(e.key)) {
                  e.preventDefault();
